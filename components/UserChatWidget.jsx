...
export default function UserChatWidget() {
    ...
    const handleIframeMessages = (event) => {
      ...
      if (event.origin === API_URL || event.origin === window.location.origin) {
        ...
        if (type === 'STREAM_RESPONSE') {
          ...
          console.error('Connection error from widget:', data);
          setIsServerAvailable(false);
        }
      }
    };
    ...
    return (
      <div className="chat-widget-container flex justify-center items-center" style={{ width: '100%', height: '500px', border: '1px solid #e0e0e0', borderRadius: '8px', overflow: 'hidden' }}>
        <div className="text-center">
          ...
          <p>Loading Change Management Assistant...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="chat-widget-container flex justify-center items-center" style={{ width: '100%', height: '500px', border: '1px solid #e0e0e0', borderRadius: '8px', overflow: 'hidden' }}>
        <div className="text-center p-6">
          ...
          <h3 className="font-medium text-lg mb-2">Cannot connect to Change Management Assistant</h3>
          <p className="text-muted-foreground mb-4">The Change Management Assistant is currently unavailable. Please try again later.</p>
          <button 
            onClick={() => {
              setIsLoading(true);
  ...
  const iframeSrc = `${API_URL}/widgets/user-widget.html?stream=true&assistantType=changeManagement`;
  ...
  return (
    <div className="chat-widget-container" style={{ width: '100%', height: '500px', border: '1px solid #e0e0e0', borderRadius: '8px', overflow: 'hidden' }}>
      <iframe 
        ...
        height="100%" 
        frameBorder="0"
        title="Change Management Assistant"
        style={{ border: 'none' }}
